...
for $i (0..1)
{
	...
	for $l (0..1)
	{
		...
		for $n (1..10)
		{
		    ...
		    open FH1,'<',"MCcoal.4sghost-IM.ctl";
...
#				$line =~ s/__GHOSTS__/0/;
			...
			if ($t == 2)
				...
				chomp $line;
                                ...
                                $line =~ s/__TREEFILE__/$simcon.$n.trees/;
                                $line =~ s/__IMAPFILE__/$simcon.$n.imap/;
                                $line =~ s/__OUTFILE__/$simcon.$n.out.noiter.txt/;
                                $line =~ s/__MCMCFILE__/$simcon.$n.noiter.mcmc/;
                                $line =~ s/__NSEQS__/$NSEQS[$m]/g;
                                $line =~ s/__THETA_BETA__/$thetabeta/;
				$line =~ s/__TAU_BETA__/$taubeta/;
#				$line =~ s/__SPLINE__/4 A B C D/;
#				$line =~ s/__GHOSTS__//;
				$line =~ s/\*t2//g;
				$line =~ s/^\*.+//g;
				if (length($line) > 5)
